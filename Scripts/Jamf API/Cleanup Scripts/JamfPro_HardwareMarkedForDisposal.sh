...
########################################################################
#            Delete computer records marked for disposal               #
...
## Report to be used to check serial numbers in ABM and release the devices that are due to be recycled
...
disposalDept="DisposalDepartment"
# Temp content
...
if [[ -s "${tempDir}/${fileName}_${date}.csv" ]]; then
    ...
    echo "Attached are the computers marked for disposal in Jamf Pro" | mailx -r address@domain -s "Jamf Pro computers marked for disposal" -a "${tempDir}/${fileName}_${date}.csv" -- address@domain
else
...
while read -r computerID; do
    ...
    if [[ "$computerDepartment" == "$disposalDept" ]]; then
        ...
        | xmllint --format --xpath  "/computer/general/name | /computer/general/serial_number | /computer/location/department | /computer/hardware/model" - | awk -F'>|<' '{print $3,$7,$11,$15}' >> "${tempDir}/${fileName}.txt"
