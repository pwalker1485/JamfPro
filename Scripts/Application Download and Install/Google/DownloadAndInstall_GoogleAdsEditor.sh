...
########################################################################
#         Download and Install Google Ads Editor (Source: DMG)         #
...
appName="Google Ads Editor"
# Target URL
targetURL="https://dl.google.com/adwords_editor/google_ads_editor.dmg"
...
if [[ -d "/Applications/${appName}.app" ]]; then
	currentInstalledVerCheck=$(find /Applications/${appName}.app/Contents/Versions/* -maxdepth 0 | awk -F '/' '{print $NF}' \
    | sed '/\..*\./s/^[^.]*\.//' | sed 's/\.[^.]*$//' 2>/dev/null)
    currentInstalledVer="1.${currentInstalledVerCheck}"
...
if [[ -f "${downloadDir}/${appName}.dmg" ]]; then
    ...
    if [[ -d "/Applications/${appName}.app" ]]; then
        ...
        chmod -R 775 "/Applications/${appName}.app"
        # Check the version
        latestInstallVerCheck=$(find /Applications/${appName}.app/Contents/Versions/* -maxdepth 0 | awk -F '/' '{print $NF}' \
        | sed '/\..*\./s/^[^.]*\.//' | sed 's/\.[^.]*$//' 2>/dev/null)
        latestInstallVer="1.${latestInstallVerCheck}"
